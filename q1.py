...
def find_strongly_dominant_eq(gamedata, playerno, totalplayer, topplayer, strategies,
    ...
    if len(totalplayer) >= 1:  # If there are remaining players to consider
        ...
        #     payoff = 0
        #     for j in range(multiplier[playerno]):
        #         sel_index = sel_index = i * multiplier[playerno] + j
        #         payoff += gamedata[sel_index]
        #     if i == strategyarr[playerno]:
        #         max_payoff = payoff
        #         max_index = i
        #     else:
...
def find_weakly_dominant_eq(gamedata, playerno, totalplayer, topplayer, strategies, 
    ...
    if len(totalplayer) >= 1:
        ...
        #     payoff = 0
        #     for j in range(multiplier[playerno]):
        #         sel_index = i * multiplier[playerno] + j
        #         payoff += gamedata[sel_index]
...
def psne_gen(num_players, strategy, util_matrix):
    ...
    ##########     Write Code          ###############
     # Iterate through all possible strategy combinations for each player
    for strategy_combination in np.ndindex(*strategy):
        is_psne = True  # Assume it's a PSNE until proven otherwise

        # Check if this strategy combination is a Nash equilibrium
        for player in range(num_players):
            player_strategy = strategy_combination[player]  # Strategy of the current player
            opponent_strategies = list(strategy_combination[:player]) + list(strategy_combination[player + 1:])  # Strategies of all opponents
            opponent_strategies_tuple = tuple(opponent_strategies)

            # Utility of the current player with their chosen strategy against opponent strategies
            player_utility = util_matrix[strategy_combination][player]

            # Check if the current player can improve their utility by deviating from their strategy
            for alt_strategy in range(strategy[player]):
                if alt_strategy != player_strategy:
                    alt_strategy_combination = list(strategy_combination)
                    alt_strategy_combination[player] = alt_strategy
                    alt_strategy_combination = tuple(alt_strategy_combination)
                    alt_player_utility = util_matrix[alt_strategy_combination][player]

                    # If utility with alternate strategy is greater, current strategy is not a PSNE
                    if alt_player_utility > player_utility:
                        is_psne = False
                        break
            if not is_psne:
                break

        # If the current strategy combination is a PSNE, add it to the PSNE list
        if is_psne:
            psnelist.append(strategy_combination)
...
def msne_gen(num_players, strategy, util_matrix):
    ...
    for player in range(num_players):
        ...
        for i in range(strategy[player]):
            ...
            best_response_probs.append(expected_utility)
        # Normalize the best response probabilities
        total_probilities = sum(best_response_probs)
        if (total_probilities == 0):
            for i in range(len(best_response_probs)):
                best_response_probs[i] += 1e-7
            total_probilities = sum(best_response_probs)
        best_response_probs = [prob / total_probilities for prob in best_response_probs]
        # Append the best response probabilities to the MSNE list for the current player
        # msne_player.append(best_response_probs)
        msne.append(best_response_probs)    
